...
# Charge la config mail depuis la racine du projet ET depuis backend/.env.
# backend/.env prend le dessus si les deux existent.
...
from .helpers import generate_ai_summary, quote_html, invoice_html, try_pdf_response
...
def init_v10_routes(templates: Jinja2Templates, storage_dir: Path, report_dir: Path, sanitize_filename, intervention_dir_fn, resolve_storage_path):
    ...
    def _render_document_attachment(html_content: str, filename: str) -> tuple[bytes, str, str, str]:
        """Génère systématiquement une pièce jointe.
        Priorité :
        1) wkhtmltopdf si disponible ;
        2) xhtml2pdf si installé via requirements ;
        3) HTML imprimable en dernier recours.
        """
        pdf_name = filename if filename.lower().endswith(".pdf") else f"{Path(filename).stem}.pdf"

        # 1) Conversion fidèle via wkhtmltopdf si présent sur le poste.
        wkhtml = None
        ...
        # 2) Conversion PDF intégrée Python. C'est celle utilisée quand wkhtmltopdf n'est pas installé.
        try:
        ...
        # 3) Dernier recours : HTML imprimable attaché.
    ...
    def _send_document_email(
        *,
        ...
        payload, maintype, subtype, final_name = _render_document_attachment(html_attachment, attachment_name)
    ...
    def _smtp_not_configured_redirect(target: str) -> RedirectResponse:
        """Retour applicatif uniquement : aucun mailto / Outlook."""
    ...
    def send_quote_email(quote_id: int, request: Request, session: Session = Depends(get_session)):
        ...
        if detail == "smtp_not_configured":
            log_activity(session, user, "quote.email_error", f"{quote_obj.quote_number} smtp_not_configured")
            session.commit()
            return _smtp_not_configured_redirect("quotes")
    ...
    def send_invoice_email(invoice_id: int, request: Request, session: Session = Depends(get_session)):
        ...
        if detail == "smtp_not_configured":
            log_activity(session, user, "invoice.email_error", f"{invoice_obj.invoice_number} smtp_not_configured")
            session.commit()
            return _smtp_not_configured_redirect("invoices")
    ...
    def update_intervention_labor(
        ...
        intervention.labor_rate = max(labor_rate, 60.0)
